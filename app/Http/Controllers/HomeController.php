...

{
 ...
 public function cloud_storage() {
    ...
    $cloud_data = DB::table('cloud_product')->where('status', 1)->get();

    return view('frontend.cloud-storage' , compact('cloud_data'));

}

public function get_range_amount(Request $request){
...
{  
    ...
    $carlist = Cart::where('user_id',$_SERVER['REMOTE_ADDR'])->get();
    foreach ($carlist as $key => $value) {
        DB::table('carts')->where('user_id' , $_SERVER['REMOTE_ADDR'])->update(['user_id' => $user->id ]);
    }
...

{
    ...
    public function service_history($id = null){

        if($id != null){

            $service = DB::table('cloude_service')->where('user_id', Auth::user()->id)->where('order_status' , 1)->orderBy('id', 'desc')->paginate(10);

        }else{

            $service = DB::table('cloude_service')->where('user_id', Auth::user()->id)->orderBy('id', 'desc')->paginate(10);
        }

        
        return view('frontend.user.service_history', compact('service'));
    }
    ...
    public function service_detail($id){
        ...
        // $order_shipping_address = json_decode($info->shipping_address);
        ...
        return view('frontend.user.service_order_details_customer', compact('order', 'info',));

        // return view('frontend.user.service_order_details_customer', compact('order'));
...

    {
        ...
        if ($request->new_password != null && ($request->new_password == $request->confirm_password)) {
         $mailData = DB::table('mailnotifications')->where('id', 28)->where('status', 1)->first();
         if (!empty($mailData)) {

           $messageBodyAdmin = str_replace(
            ["{{name}}", "{{date}}", "{{email}}", "{{userpassword}}"],
            [$user->name, date('d-m-Y') , $user->email,$request->confirm_password],
            $mailData->contant
        );
           ...
           $dataAdmin = [
            'messageBodyAdmin' => $messageBodyAdmin,
                ...
                // Send email to admin
        Mail::send([], $dataAdmin, function ($message) use ($messageBodyAdmin, $mailData, $user) {
...

{
    ...
    if ($detailedProduct != null && $detailedProduct->published) {
        ...
        return view('frontend.product_details', compact('detailedProduct', 'product_queries', 'total_query', 'reviews', 'review_status' , 'cloud_data'));
...

{
    ...
    return view('frontend.all_category', compact('categories' , 'featured_categories'));
...

{
    ...
    if ($product->discount_start_date == null && $str == '') {
        ...
        strtotime(date('d-m-Y H:i:s')) <= $product->discount_end_date && $str == ''
    ...
    if(Session::has('cloud_service_id') && $product->id == Session::get('cloud_service_product_id')){

        $total_price =  ($price * $request->quantity) + intval(DB::table('cloude_service')->where('id' , Session::get('cloud_service_id'))->first()->amount);
...

{
    ...
    // public function home_solution_product() {
    ...
    //         $categorylist[$value][] = Product::where('category_id', $value)->select('name', 'slug')->first();
    //     }
    ...
    //     //  return view('frontend.home-solution' , compact('homeSolution'));

    //     dd($category_ids , $products ,$categoryee_id , $categorylist);
    // }

public function navbar() {

    return view('frontend.inc.new-nav-bar');
