...
function get_best_selling_products_for_cart()

{
    ...
    return $product_query->orderBy('num_of_sale', 'desc')->limit(5)->get();
...
if (!function_exists('get_user_cloud_service')) {

    function get_user_cloud_service()

    {
        ...
        if (auth()->user() != null) {
            ...
            $cloud = DB::table('cloude_service')->where('user_id' , Auth::user()->id)->where('payment_status' , 0)->get();


        } else {
              
            $user_id = request()->ip();
            $cloud = DB::table('cloude_service')->where('user_id' , $user_id)->where('payment_status' , 0)->get();
...
function get_home_solution($id)
{
    ...
    $categoryee_id = array_map(function ($id){

        return $id['category_id'];

    },$products->toArray());

    foreach ($categoryee_id as $value) {
        ...
        if(array_key_exists($value, $categorylist)){
            ...
            $categorylist[$value]= Product::where('category_id', $value)->select('name', 'thumbnail_img', 'slug')->get();
        }
...
function get_bundle_discount() {
    ...
    $carts = $carts ?? Cart::where('user_id', $user_id)->get();
    ...
    foreach ($carts as $value) {
        ...
        if ($value->addon_product_id == null) {

            $product_ids[] = $value->product_id;

        } else {

            $addon_product_ids[] = $value->addon_product_id;

        }
        
         $product = Product::find($value->product_id);

         $subtotals += cart_product_price($value, $product, false, false) * $value->quantity;

         $tax +=  cart_product_tax($value, $product, false) * $value->quantity;
    ...
    // dd($subtotals,$totprice );

    foreach ($addon_product_ids as $addon_product_id) {

        if (in_array($addon_product_id, $product_ids)) {
    ...
    if (Session::has('cloud_service_id')) {
        ...
        $cart_detail = DB::table('cloude_service')->where('user_id', $user_id)->where('payment_status', 0)->first();

        $subtotals += $cloud_amount ?? $cart_detail->amount;

    }

    $discount_id = 0;

    if ($addon_check && Session::has('cloud_service_id')) {

        $discount_id = 3;

    } elseif ($addon_check && $cart_detail == null) {

        $discount_id = 1;

    } elseif (!$addon_check && (Session::has('cloud_service_id'))) {
    ...
    if($discount_id != 0){
        ...
        $discount = DB::table('discount')->where('id', $discount_id)->first();
    ...
    if ($discount) {
        ...
        if ($discount->discount_type == 'Percent') {
            ...
            $discountAmount = ($subtotals * $discountPercentage) / 100;

            $grandTotal = ($subtotals - $discountAmount) + $tax + $shipping + $coupon_discount;
          ...
          CombinedOrder::where('id', Session::get('combined_order_id'))->update(['grand_total' => $grandTotal]);
        ...
        $grandTotal = $subtotals + $tax + $shipping + $coupon_discount;
    }

    $data =['bundle_discount' => $discount_price , 'total_amount' => $grandTotal];
    // dd($grandTotal , $discount_price);
    return $data;
}
...
function get_order_commission($current = false , $last = false) {
    ...
    if($current){
        ...
        $products = OrderDetail::join('orders', 'orders.id', '=', 'order_details.order_id')
    ...
    foreach ($products as $productId) {
        ...
        if ($product) {
            ...
            $totalPrice += home_discounted_base_price($product, false);

        }
    }

    if($totalproduct >= 1 &&  $totalproduct <= 500){

        $silver     = BusinessSetting::where('type', 'silver')->first()->value ?? 0 ;

        $commission_price = ($totalPrice * $silver) / 100;

    }elseif($totalproduct >= 501 &&  $totalproduct <= 1000){

        $gold       = BusinessSetting::where('type', 'gold')->first()->value ?? 0;

        $commission_price = ($totalPrice * $gold) / 100;

    }elseif($totalproduct <= 1001){

        $diamond    = BusinessSetting::where('type', 'diamond')->first()->value ?? 0;

        $commission_price = ($totalPrice * $diamond) / 100;
    }

    return format_price(convert_price($commission_price));
}

function get_service_commission($current = false , $last = false) {
    ...
    if($current){
         ...
         $amounts = DB::table('cloude_service')
        ...
        ->where('cloude_service.payment_status' , 2)

        ->select('cloude_service.amount')
    ...
    if($totalPrice >= 1 &&  $totalPrice <= 5000){

        $silver     = BusinessSetting::where('type', 'se_silver')->first()->value ?? 0 ;

        $commission_price = ($totalPrice * $silver) / 100;

    }elseif($totalPrice >= 5001 &&  $totalPrice <= 10000){

        $gold       = BusinessSetting::where('type', 'se_gold')->first()->value ?? 0;

        $commission_price = ($totalPrice * $gold) / 100;

    }elseif($totalPrice <= 10001){

        $diamond    = BusinessSetting::where('type', 'se_diamond')->first()->value ?? 0;
...
function get_total_order_commission($userid=false, $givenprice=false) {
    
    $Price = ($givenprice) ? $givenprice : 0;
    ...
    // $partner_code = DB::table('users')->where('id' , $user_id)->select('partner_code')->first()->partner_code;
    ...
    if($totalproduct >= 1 &&  $totalproduct <= 500){
        ...
        $commission_price = ($Price * $silver) / 100;

    }elseif($totalproduct >= 501 &&  $totalproduct <= 1000){

        $gold       = BusinessSetting::where('type', 'gold')->first()->value ?? 0;

        $commission_price = ($Price * $gold) / 100;

    }elseif($totalproduct <= 1001){

        $diamond    = BusinessSetting::where('type', 'diamond')->first()->value ?? 0;

        $commission_price = ($Price * $diamond) / 100;
    }
    ...
    if($givenprice){
        ...
        $service_commission =  get_total_service_commission ();

        $payamount = Payment::where('seller_id' , $user_id)->get();
    
        if(count($payamount) > 0){
    
            $total_pay_amount = 0;
    
            foreach($payamount as $payprice){
    
                $total_pay_amount += $payprice->amount;
    
            }
    
            $totalprice = $commission_price + $service_commission - $total_pay_amount;
    
            Shop::where('user_id' , $user_id)->update(['admin_to_pay' => $totalprice]);
    
        }else{
           
            $totalprice = $commission_price + $service_commission;
...
function get_total_service_commission($userid=false, $givenprice=false) {
    ...
    if($totalPrice >= 1 &&  $totalPrice <= 5000){
        ...
        $commission_price = (( ($givenprice) ? $givenprice :  $totalPrice) * $silver) / 100;

    }elseif($totalPrice >= 5001 &&  $totalPrice <= 10000){

        $gold       = BusinessSetting::where('type', 'se_gold')->first()->value ?? 0;

        $commission_price = (( ($givenprice) ? $givenprice :  $totalPrice) * $gold) / 100;

    }elseif($totalPrice <= 10001){

        $diamond    = BusinessSetting::where('type', 'se_diamond')->first()->value ?? 0;

        $commission_price = (( ($givenprice) ? $givenprice :  $totalPrice) * $diamond) / 100;
    }

    return $commission_price;
}

function get_grand_total_commission($req){
   
    $orders =  Order::whereHas('orderDetails', function ($query) {

        $query->where('product_referral_code', '!=', '');
    ...
    }else if($req == 'lastmonth') {

        $orders = $orders->whereMonth('created_at' ,  date('m')-1);

    }else if($req == 'Year'){
        
        $orders = $orders->whereMonth('created_at' ,  date('Y'));

    }else if($req == 'lastyear'){

        $orders = $orders->whereMonth('created_at' ,  date('Y')-1);
    }
    ...
    foreach($orders as $order){
        ...
        $id = DB::table('users')->where('partner_code', base64_decode($order->orderDetails[0]['product_referral_code']))->select('id')->first();

        $totalcommssion += get_total_order_commission($id->id , $order->grand_total);

    }

    $total = $totalcommssion + get_grand_total_service_commision($req);

    return $total;
}

function get_grand_total_service_commision($req) {
    ...
    foreach($orders as $order){
        ...
        $totalcommssion += get_total_service_commission($order->user_id , $order->amount);
...
function get_growth($fval , $sval){

   return number_format((( intval($fval) - intval($sval))/$sval) * 100 ,  2);
   
}

function num_of_sale($numofsales){

  $num =  filter_products(\App\Models\Product::where('published', 1)->where('num_of_sale' ,'!=','0')->orderBy('num_of_sale', 'desc'))->select('num_of_sale')->get();

  $total_sales = 0;

  foreach ($num as $value) { $total_sales += $value->num_of_sale; };

  $per = number_format(($numofsales/$total_sales)*100 , 2);
   
  return $per;

}

function get_category_list($id , $limt = false) {

    $categories = Category::where('parent_id', $id)->where('digital', 0)->with('childrenCategories');
    ...
    if($limt){
        ...
        $categories = $categories->latest()->take($limt);
    }
...
function get_top_selling_product($id , $limt = false) {

    $product = Product::where('published', 1)->where('category_id' , $id)->orderBy('num_of_sale', 'desc')->limit($limt)->get();
    
    return $product;

}

function get_unpaid_service() {

   $service = DB::table('cloude_service')->where('user_id' , Auth::user()->id)->where('order_status' , 1)->first();

   return $service;
}

function get_new_product($limt = false) {

    $product = Product::where('published', 1)->orderBy('created_at', 'desc')->limit($limt)->get();
    
    return $product;

}

function get_new_product_home($limt = false) {

    $product = Product::where('published', 1)->where('category_id' ,'94')->orderBy('created_at', 'desc')->limit($limt)->get();
    
    return $product;

}

function get_new_product_business($limt = false) {

    $product = Product::where('published', 1)->where('category_id' ,'96')->orderBy('created_at', 'desc')->limit($limt)->get();
    
    return $product;

}

function get_new_product_smart($limt = false) {

    $product = Product::where('published', 1)->where('category_id' ,'97')->orderBy('created_at', 'desc')->limit($limt)->get();
