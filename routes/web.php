...
use App\Http\Controllers\Payment\AchController;
...
Route::post('states', [ShopController::class, 'states'])->name('states');

Route::post('cities', [ShopController::class, 'cities'])->name('cities');

//renew

Route::controller(RenewController::class)->group(function () {

    Route::get('renew/{id}', 'index')->name('renew');
    
});

Route::get('allcategories', [ProductController::class, 'allcategories'])->name('allcategories');
...
Route::controller(HomeController::class)->group(function () {
    ...
    Route::get('/cloud-storage', 'cloud_storage')->name('cloud_storage');
    Route::post('/guestuser', 'guestuser')->name('user.guestuser');
    ...
    Route::get('/home/home-solution-product' , 'home_solution_product')->name('home-solution-product');
    ...
    Route::get('/nav-bar' , 'navbar')->name('home.navbar');
...
Route::controller(CartController::class)->group(function () {
    ...
    Route::post('/cart/removeFromCartaddon', 'removeFromCartaddon')->name('cart.removeFromCartaddon');

    Route::post('/cart/updateQuantity', 'updateQuantity')->name('cart.updateQuantity');

    Route::post('/cart/store-cloud-service', 'store_cloud_service')->name('cart.store-cloud-service');

    Route::post('/cart/check-store-cloud-service', 'check_cloud_service')->name('cart.check-cloud-service');
...
Route::controller(PaypalController::class)->group(function () {
    ...
    Route::post('/paypal/payment/done', 'getDone')->name('payment.done');

    Route::get('/paypal/payment/cancel', 'getCancel')->name('payment.cancel');

});

Route::post('/api/orders', [PayPalController::class, 'createOrder'])->name('paypal.create-order');

Route::post('/api/orders/{orderID}/capture', [PayPalController::class, 'captureOrder'])->name('paypal.capture-order');
//Mercadopago START
...
Route::controller(AchController::class)->group(function () {

    Route::get('/ach/success', 'success')->name('ach.success');
    
    Route::get('/ach/cancel', 'cancel')->name('ach.cancel');
});
//Stripe END
...
Route::group(['middleware' => ['user', 'verified', 'unbanned']], function () {
    ...
    Route::controller(HomeController::class)->group(function () {
        ...
        Route::get('/service-history/{id?}', 'service_history')->name('service-history');

        Route::get('/service-detail/{id}', 'service_detail')->name('service-detail');
...
Route::group(['prefix' => 'checkout'], function () {
    ...
    Route::controller(CheckoutController::class)->group(function () {
        ...
        Route::get('/payment-select', 'store_delivery_info')->name('checkout.store_delivery_info');

        Route::post('/process-payment/{amount?}/{id?}', 'check')->name('checkout.process.payment');

        Route::post('/process-payment2/{amount?}/{id?}', 'check2')->name('checkout.process.payment2');

        Route::get('/order-confirmed', 'order_confirmed')->name('order_confirmed');

        Route::post('/payment', 'checkout')->name('payment.checkout');
        
        Route::post('/payment_paypal', 'checkout_paypal')->name('payment.checkout_paypal');
        ...
        Route::get('ach-enroll' , 'ach_enroll')->name('user.ach.enroll');

        Route::post('auto-pay' , 'auto_pay')->name('checkout.user.auto.pay');
...
Route::group(['middleware' => ['auth']], function () {
    ...
    Route::get('service_invoice/{order_id}', [InvoiceController::class, 'service_invoice_download'])->name('service_invoice.download');
...
Route::post('/create-account' , [AchController::class , 'Store'])->name('acount.store');

Route::get('/pay' , [AchController::class , 'pay'])->name('pay');
