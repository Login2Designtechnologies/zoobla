...
<div class="aiz-card-box h-auto bg-white pt-3 hov-scale-img">
    <div class="position-relative img-fit overflow-hidden">
        ...
        {{-- <a class="cart-btn absolute-bottom-left w-100 h-35px aiz-p-hov-icon text-white fs-13 fw-700 d-flex flex-column justify-content-center align-items-center @if (in_array($product->id, $cart_added)) active @endif"
                ...
                @if (Auth::check()) onclick="showAddToCartModal({{ $product->id }})" @else onclick="showLoginModal1()" @endif>
                <span class="cart-btn-text">
    ...
    <div class="text-left new-bg">
        ...
        <div class="fs-14 d-flex justify-content-center mt-3">
            @if ($product->auction_product == 0)
                ...
                <!-- price -->
                <div class="">
                    <span class="fw-700 text-blue">{{ home_discounted_base_price($product) }}</span>
                </div>
            @endif
            @if ($product->auction_product == 1)
                <!-- Bid Amount -->
                <div class="">
                    <span class="fw-700 text-blue">{{ single_price($product->starting_bid) }}</span>
                </div>
            @endif
        </div>
    </div>
