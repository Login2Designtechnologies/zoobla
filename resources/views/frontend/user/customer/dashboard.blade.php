...
<style>
    ...
    .custom-shadow {
        box-shadow: rgb(208, 239, 254) 0px 7px 20px 0px;
        border: 1px solid aliceblue;
    }
    .custom-shadow.w-100.your_bill{
        background-image: radial-gradient(farthest-corner at 60% 55%, #f2fafe,#a2e0fe);
        box-shadow: rgb(190, 233, 254) 0px 7px 20px 0px;
    }
    .view-bill{
        margin-top:20px!important;
    }
    [class*="border"]{
        border-color: #3975b0 !important;
    }
    .aiz-user-panel .custom-shadow .ul-li li a:focus,.aiz-user-panel .custom-shadow .ul-li li a:hover{
        background-color: #1fade5;
        color: #fff !important;
        text-decoration:none;
        /* border-radius: 20px; */
    }
    .aiz-user-panel .custom-shadow .ul-li li{
       
        border-bottom: 2px solid #e3e3e3;
    }
    .aiz-user-panel .custom-shadow .ul-li li:last-child{
        border-bottom: none;
    }
    /* .aiz-user-panel .custom-shadow h4.fs-20{
        animation-name: txt-color;
    ...
    @keyframes txt-color{
        0%   {color: #3490f3;}
        25%  {color: rgb(145, 152, 229);}
        50%  {color: #9772e0;}
        100% {color: #3490f3;}
    }  */
</style>
@php
    $subscripion = DB::table('stripe_subscription')
                 ->join('cloude_service' ,'cloude_service.id' ,'=' ,'stripe_subscription.cloud_service_id')
                 ->where('stripe_subscription.user_id' , Auth::user()->id)
                 ->where('stripe_subscription.sub_scheduled_id' , '!=', NULL)
                 ->select('stripe_subscription.id', 'cloude_service.id as service_id', 'cloude_service.amount', 'stripe_subscription.end_date')
                 ->first();

    $service_id =  $subscripion->service_id ?? get_unpaid_service()->id ?? null;
@endphp
    <div class="row gutters-16">
        ...
        <div class='row'>
            <div class='col-8 d-flex'>
                <div class='custom-shadow w-100 your_bill pt-3'>
                    <div class='row align-items-center'>
                        <div class="col-6 border-right">
                            <div class="h-100">
                                <div class="row h-100 @if(get_setting('wallet_system') != 1 && addon_is_activated('club_point')) row-cols-md-2 @endif row-cols-1">
                                    <!-- Expenditure summary -->
                                    <div class="col">
                                        <div class="p-4 @if(!addon_is_activated('club_point')) h-100 @endif">
                                            <div class="d-flex align-items-center pb-4 ">
                                                ...
                                                <div class="d-flex flex-column justify-content-between lh-base">
                                                    <span class="fs-20 fw-500 text-dark mb-1">{{ translate('Your Bill') }}</span>
                                                    <span class="fs-16 fw-400 text-grey mb-1">Amount Due</span>
                                                    <span class="fs-20 fw-700 text-dark">{{single_price($subscripion->amount ?? (get_unpaid_service()->amount ?? 00)) }}</span>
                                                </div>
                                            </div>
                                            <div class=''>
                                                <div class='mb-4'>
                                                    <a href="javascript:void(0)" onclick="enroll_autopay()" class='btn btn-custom'>Enroll in Auto Pay</a></div>
                                                <small>You're all set,Your Auto Pay is scheduled for <b>@if($subscripion != null) {{date("Y-m-d" , $subscripion->end_date )}} @else {{date("Y-m-d")}} @endif</b></small>
                                                <div class='mt-5 w-100'>
                                                    @if($service_id != null)
                                                    <a href="{{route('renew' , encrypt($service_id))}}" class='btn btn-primary w-100 '>Make Payment</a>
                                                    @else
                                                    <a href="javascript:void(0)" class='btn btn-primary w-100 '>Make Payment</a>

                                                    @endif
                                                </div>
                                            </div>
                                            <!-- <a href="{{ route('purchase_history.index') }}" class="fs-12 text-dark">
                        ...
                        <div class='col-6'>
                            <div class="lh-lg ml-3">
                                <!-- <span>Bill Details</span> -->
                                <h5 class="fs-20 fw-500 text-dark mb-2 pb-3">Bill Details</h5>
                                <h6 class=''>What's in your statement?</h6>
                                <small>view an breakdown of your statement.</small>
                                <!-- <a href="javascript:void(0)" class='new-blue'>View Billing</a> -->
                                <a href="{{url('purchase_history')}}" class='btn btn-primary text-white view-bill'>View Billing</a>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <div class='col-4 d-flex'>
               <div class='custom-shadow w-100 p-4'>
                    <!-- <span class='fs-20'>Quick Links</span> -->
                    <h4 class="fs-20 w-100 text-center">Quick Links</h4>
                    <ul class='ul-li p-0 lh-lg'>
                        <li class='new-blue'><a href="javascript:void(0)" class="w-100 pl-3">Cancel</a></li>
                        <li class='new-blue'><a href="{{url('/profile#Address_add')}}" class="w-100 pl-3">Change address</a></li>
                        <li class='new-blue'><a href="{{url('/checkout')}}" class="w-100 pl-3">Change payment</a></li>
                        <li class='new-blue'><a href="{{route('service-history',encrypt(1))}}" class="w-100 pl-3">Renew now</a></li>
                        <li class='new-blue'><a href="{{url('/profile')}}" class="w-100 pl-3">Edit Account</a></li>
                        <li class='new-blue'><a href="{{url('/profile#Address_add')}}" class="w-100 pl-3">Edit Address</a></li>
                        <li class='new-blue'><a href="{{route('service-history')}}" class="w-100 pl-3">View Service Detials</a></li>
                        <li class='new-blue'><a href="{{url('/logout')}}" class="w-100 pl-3">Logout</a></li>
                    </ul>
               </div>
            </div>
        </div>
...
@section('modal')
    ...
    <script type="text/javascript">
        ...
        function enroll_autopay(){

            $.ajax({

                url: "{{route('checkout.user.auto.pay')}}",
